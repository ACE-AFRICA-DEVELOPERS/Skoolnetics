    ...
    makeLessonNote = async (req, res, next) => {
        try{ 
            if(req.session.staffCode){
                ...
                res.render('make-lesson-note', {title: "Lesson Notes", staff: staff, code : school.schoolCode, session: session,
    ...
    getUploadNote = async(req, res, next) => {
        try{ 
            if(req.session.staffCode){
                ...
                res.render('upload-lesson-note', {title: "Lesson Notes", staff: staff, code : school.schoolCode, session: session,
                    term: term, notes_active : "active", lessonNote : lessonNote, sessS: session.name, termS: term.name,
                    lessonNote_active : "active", note_active : "pcoded-trigger", make_active : "active"})
    ...
    postUploadNote = async(req, res, next) => {
        try{
            if(req.session.staffCode){
                ...
                const session = await Session.findOne({school : school._id, current : true})
                const term = await Term.findOne({session : session._id, current: true})
                const{subject, week, className, date} = req.body
                ...
                if(staff){
                    if(req.file){
                        ...
                        if(ret){
                            ...
                            const redirectUrl = '/staff/lesson-note/' + req.params.term + '/all'
                            res.redirect(303, redirectUrl)
                        }else{
                            throw{
                                message : "Cannot add Lesson Note"
        ...
        try{ 
            if(req.session.staffCode){
                ...
                if(lessonNote.length > 0){
                    ...
                    note_active : "pcoded-trigger", all_active : "active"})
                }else{
                    res.render("view-lesson-note", {title: "Lesson Notes", staff: staff, code : school.schoolCode, noNote: "No note has been created.",
                    lessonNote_active : "active", session: session, term : term, sessS: session.name, termS: term.name,
                    note_active : "pcoded-trigger", all_active : "active"})
                }
            }else{
            ...
            if(req.session.staffCode){
                ...
                if(singleLessonNote){
                    res.render("view-single-lesson-note", {title: "Lesson Notes", staff: staff, school : school.schoolCode,
                        singleLessonNote : singleLessonNote, notes_active: "active", term: term, session: session, 
    ...
    getLessonNotes = async (req, res, next) => {
        try{
            if(req.session.regNumber){
                ...
                const term = await Term.findOne({session: session._id, current: true})
                const lessonNotes = await LessonNote.find({session : session._id, term : term._id, status : "Approved"})
                ...
                if(lessonNotes.length > 0){
                    res.render('lesson-notes', {title : "Lesson Notes", student: student, 
    ...
    getSingleNote = async (req, res, next) => {
        try{
            if(req.session.regNumber){
                ...
                res.render('student-note', {title : "Lesson Note", student: student, 
                    ...
                    lessonNote_active : "active", termS: term.name, note_active : "pcoded-trigger", make_active : "active"})
            }else{
                res.redirect(303, '/school')
            }
        }catch(err){
