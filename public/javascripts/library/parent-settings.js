...
submitPassword.addEventListener("click" , event => { 
    ...
    if(oldPassword.value != '' && newPassword.value != '' && cNewPassword.value != ''){
        if(newPassword.value == cNewPassword.value){
            sendData('/change-parent-password', {oldPassword: oldPassword.value, newPassword: newPassword.value, cNewPassword: cNewPassword.value})
            .then(res => {
                if(res.status > 400){
                    ...
                    setTimeout(() => {
                        window.location.replace('/parent/settings') 
                    } , 1000)
                }
            })
            .catch(err =>{
